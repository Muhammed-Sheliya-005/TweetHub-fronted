...
export default function Dashboard() {
  ...
  const [view, setView] = useState("all"); // "all" or "profile"
  ...
  const fetchAllTweets = async () => {
    try {
      const res = await axios.get(`${base}/api/tweets`);
  ...
  const fetchMyTweets = async () => {
    if (!user) return;
    try {
      const res = await axios.get(`${base}/api/tweets?userId=${user.id}`);
  ...
  // Decide which tweets to show
  const tweetsToShow = view === "all" ? allTweets : myTweets;

  return (
    <>
      {/* Pass view and setView to Navigation for toggle */}
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        {loading ? (
          ...
          <p className="text-center text-gray-500">
            {view === "all" ? "No tweets yet!" : "You haven't posted any tweets yet!"}
          </p>
        ) : (
          tweetsToShow.map((tweet) => (
            <div
              key={tweet.id}
              className="p-4 mb-4 border border-gray-200 rounded-lg shadow-sm bg-white"
            >
